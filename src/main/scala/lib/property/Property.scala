 ...
 * class has many mutable properties, such as `hp`, `position` etc.
  ...
  *   }}}
  */
final case class Property[T](initialValue: T, setter: T => T = (t: T) => t, getter: T => T = (t: T) => t) {

  private val onSetController = EmitterController[(T, T)]()
  ...
  /** Emitted whenever the value of this property is set — even when the new
    ...
    */
  val onSet: Emitter[(T, T)] = onSetController.emitter

  private val onChangeController = EmitterController[(T, T)]()
  ...
  /** Emitted whenever the value of this property is set to a new value that is
    ...
    * Emit data contains a tuple with the previous and new values.
    */
  val onChange: Emitter[(T, T)] = onChangeController.emitter
